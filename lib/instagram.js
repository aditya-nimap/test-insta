...
// Stories container — media_type must be "STORIES" (not "IMAGE"/"REELS").
// No caption field: Stories don't support captions via the API.
// Best aspect ratio: 9:16 vertical. Video: MP4/H.264, max 60 seconds.
export async function createStoryContainer({
  igUserId,
  accessToken,
  mediaUrl,     // publicly reachable HTTPS URL
  ...
  const data = await res.json();
  if (!res.ok) {
    throw new Error(`Story container creation failed: ${JSON.stringify(data)}`);
  }
  return data.id; // same container id flow as regular posts
}
